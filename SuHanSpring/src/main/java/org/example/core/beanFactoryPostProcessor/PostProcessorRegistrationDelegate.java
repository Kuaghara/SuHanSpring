...
public class PostProcessorRegistrationDelegate {
    ...
    public static void invokeBeanFactoryPostProcessors(DefaultListableBeanFactory beanFactory){
       ...
       sortPostProcessors(priorityOrderedPostProcessors);
        ...
        AnnotationUtil.beanDefinitionSort(beanDefinitionList, beanFactory);
        for(BeanDefinition bd : beanDefinitionList){
            if(BeanPostProcessor.class.isAssignableFrom(bd.getClazz())){
            BeanPostProcessor beanPostProcessor = beanFactory.getBean(bd.getClassName(), BeanPostProcessor.class);
