...
public class AbstractDefaultListableBeanFactory implements AbstractFactory {
        ...
        try {
            if (singltonObject == null) {
                ...
                try {
                    ...
                    //下面一行以及两行注释为ai编写
                   // 缓存完全创建的单例，使后续的getBean（）调用返回同一个实例。
                    //如果没有这一点，后处理器可以在刷新（）期间多次实例化。
                    registry.registerSingleton(name, singltonObject);
                } finally {
                    //此行为ai添加
                    singletonCurrentlyInCreation.remove(name);
                }
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    public Object instantiationBean(BeanDefinition bd) {
        ...
        try {
            ...
            throw new RuntimeException("Failed to instantiate bean: " + bd.getClassName(), e);
        } catch (InvocationTargetException e) {
            Throwable target = e.getTargetException();
            throw new RuntimeException("Failed to instantiate bean: " + bd.getClassName(), target != null ? target : e);
        }
